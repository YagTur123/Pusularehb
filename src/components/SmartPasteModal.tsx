...
export function SmartPasteModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="bg-slate-900 border border-slate-700/80 rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] flex flex-col overflow-hidden text-slate-200">
        {/* Header */}
        ...
        <div className="p-6 flex-1 overflow-y-auto space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full p-3 rounded-lg bg-slate-950 border border-slate-800 text-white font-mono text-xs focus:outline-none focus:border-indigo-500 selection:bg-indigo-950 resize-y"
            />
          </div>

          {/* Real-time stats */}
          {parsedRows.length > 0 && (
            <div className="flex items-center gap-4 text-xs font-mono py-1 px-3 bg-slate-950/60 rounded-md border border-slate-800">
              <span className="text-slate-400">
                Toplam Satır: <strong className="text-white">{parsedRows.length}</strong>
              </span>
              <span className="flex items-center gap-1 text-emerald-400">
                <CheckCircle2 className="w-3.5 h-3.5" />
                Geçerli: <strong className="text-white">{validRows.length}</strong>
              </span>
              {invalidRows.length > 0 && (
                <span className="flex items-center gap-1 text-rose-400">
                  <AlertTriangle className="w-3.5 h-3.5" />
                  Eksik/Hatalı: <strong className="text-white">{invalidRows.length}</strong>
                </span>
              )}
            </div>
          )}
          ...
          {parsedRows.length > 0 && (
            <div className="border border-slate-800 rounded-lg overflow-hidden">
              <div className="bg-slate-950 px-4 py-2 border-b border-slate-800 text-xs font-semibold text-slate-300">
                Ayrıştırılan Öğrenci Önizlemesi ({validRows.length} / {parsedRows.length})
              </div>
              <div className="max-h-60 overflow-y-auto">
                <table className="w-full text-left text-xs">
                  <thead className="bg-slate-950/80 text-slate-400 border-b border-slate-800/80 font-mono text-[11px] sticky top-0">
                    <tr>
                    ...
                    {parsedRows.map((row, idx) => (
                      <tr
                        ...
                        className={`hover:bg-slate-800/40 transition-colors ${
                          !row.valid ? 'bg-rose-950/10' : ''
                        }`}
                      >
                        <td className="px-3 py-2">
                          {row.valid ? (
                            <span className="inline-flex items-center gap-1 text-[10px] text-emerald-400 font-mono bg-emerald-500/10 px-1.5 py-0.5 rounded border border-emerald-500/20">
                              <CheckCircle2 className="w-3 h-3" /> Hazır
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1 text-[10px] text-rose-400 font-mono bg-rose-500/10 px-1.5 py-0.5 rounded border border-rose-500/20" title={row.error}>
                              <AlertTriangle className="w-3 h-3" /> {row.error || 'Hata'}
                            </span>
                          )}
                        </td>
                        <td className="px-3 py-2 font-medium text-white">
                          {row.full_name || <span className="text-slate-500">-</span>}
                        </td>
                        <td className="px-3 py-2 font-mono text-slate-300">
                          {row.class_grade}
                        </td>
                        <td className="px-3 py-2 font-mono text-slate-400">
                          {row.phone ? `+${row.phone}` : <span className="text-rose-400 font-sans">Yok</span>}
                        </td>
                        <td className="px-3 py-2">
        ...
        <div className="flex items-center justify-between px-6 py-3.5 border-t border-slate-800 bg-slate-950/60">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-1.5 rounded-md text-xs font-semibold bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed text-white shadow-sm transition-all"
            >
